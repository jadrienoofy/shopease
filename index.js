...
document.addEventListener("DOMContentLoaded", () => {
        ...
        if (product) {
            ...
            document.getElementById("product-container").innerHTML = `
                <div>
                    <img src="${product.image}" alt="${product.name}" class="w-full h-[400px] object-contain">
                </div>
                <div>
            ...
            // Related Products
            const related = Object.entries(products).filter(([key]) => key !== productKey);
            related.forEach(([key, item]) => {
                document.getElementById("related-container").innerHTML += `
                    <div class="bg-white h-[400px] w-[250px] flex-shrink-0 p-2 border-2 border-transparent hover:border-gray-900 snap-center cursor-pointer"
                        ...
                        <img src="${item.image}" alt="${item.name}" class="h-[280px] mx-auto mb-6">
